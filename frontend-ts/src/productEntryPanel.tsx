    ...
    <>
      ...
      <section className="marginStageTs" aria-labelledby="margin-stage-title">
        <div className="stageHeaderTs compactStageHeader">
          ...
          <h2 className="stageTitleTs" id="margin-stage-title">Margem padrao da sessao</h2>
        </div>
        <button
          className="marginControlButtonTs fullWidthToolButton"
          type="button"
          onClick={() => void runBusyAction("margem", onApplyMargin)}
          aria-label={`Alterar margem padrao da sessao. Valor atual ${marginPercentual.toFixed(1)} por cento`}
          title="Alterar margem padrao da sessao"
        >
          ...
          <span className="marginBadgeTs">{marginPercentual.toFixed(1)}%</span>
        </button>
      </section>
    </>
  );
